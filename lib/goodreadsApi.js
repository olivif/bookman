        ...
        if (!error && response.statusCode == 200) {
            parseString(body, function (err, result) {
                ...
                if (unpackedResult === null || unpackedResult === undefined) {
                    ...
                    var shelves = [];
                    unpackedResult.forEach(function (goodreadsShelf) {
                        shelves.push(Shelf(goodreadsShelf));
                    }, this);
        ...
        if (!error && response.statusCode == 200) {
            parseString(body, function (err, result) {
                ...
                if (unpackedResult === null || unpackedResult === undefined) {
                    ...
                    var books = [];
                    unpackedResult.forEach(function (goodreadsBook) {
                        books.push(Book(goodreadsBook));
                    }, this);
